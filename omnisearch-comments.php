...
class Jetpack_Omnisearch_Comments extends WP_Comments_List_Table {
	...
	function search( $results, $search_term ) {
		...
		$search_link = sprintf( ' <a href="%s" class="add-new-h2">%s</a>', $search_url, esc_html__('Search Comments', '__textdomain__') );
		$html = '<h2>' . esc_html__('Comments', '__textdomain__') . $search_link . '</h2>';
		parent::__construct();
		...
		$label = __( 'Comments', '__textdomain__' );
		$results[ $label ] = $html;
		return $results;
	}
