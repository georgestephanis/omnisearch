...
class WP_Omnisearch_Posts extends WP_List_Table {
	var $post_type = 'post',
	    $post_type_object;
	...
	function get_columns() {
		$columns = array(
		#	'id' => __('ID', '__textdomain__'),
			'post_title' => __('Title', '__textdomain__'),
			'snippet' => __('Snippet', '__textdomain__'),
			'date' => __('Date', '__textdomain__'),
		);
		return $columns;
	}
	...
	function column_post_title( $post ) {
		...
		if ( current_user_can( $this->post_type_obj->cap->delete_post, $post ) ) {
			$actions['delete'] = sprintf( '<a href="%s">%s</a>', esc_url( get_delete_post_link( $post->ID ) ), esc_html__('Trash', '__textdomain__') );
		}
	...
	function column_date( $post ) {
		...
		if ( '0000-00-00 00:00:00' == $post->post_date ) {
			$t_time = $h_time = __('Unpublished', '__textdomain__');
			$time_diff = 0;
		} else {
			$t_time = date( __('Y/m/d g:i:s A', '__textdomain__'), mysql2date( 'G', $post->post_date ) );
			...
			if ( $time_diff > 0 && $time_diff < DAY_IN_SECONDS )
				$h_time = sprintf( __('%s ago', '__textdomain__'), human_time_diff( $time ) );
			else
				$h_time = mysql2date( __('Y/m/d', '__textdomain__'), $m_time );
		}

		$html .= '<abbr title="' . esc_attr( $t_time ) . '">' . esc_html( $h_time ) . '</abbr>';
		$html .= '<br />';
		if ( 'publish' == $post->post_status ) {
			$html .= esc_html__('Published', '__textdomain__');
		} elseif ( 'future' == $post->post_status ) {
			if ( $time_diff > 0 )
				$html .= '<strong class="attention">' . esc_html__('Missed schedule', '__textdomain__') . '</strong>';
			else
				$html .= esc_html__('Scheduled', '__textdomain__');
		} else {
			$html .= esc_html__('Last Modified', '__textdomain__');
		}
		return $html;
	}
