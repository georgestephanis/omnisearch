...
class WP_Omnisearch_Plugins extends WP_Plugin_Install_List_Table {
	static $instance;
	...
	function search( $results, $search_term ) {
		...
		$search_link = sprintf( ' <a href="%s" class="add-new-h2">%s</a>', $search_url, esc_html__('Search Plugins', '__textdomain__') );
		$html = '<h2>' . esc_html__('Plugins', '__textdomain__') . $search_link . '</h2>';

		$html .= '<div id="' . __CLASS__ . '_results">' . esc_html__('Loading &hellip;', '__textdomain__') . '</div>';
		...
		$label = __( 'Plugins', '__textdomain__' );
		$results[ $label ] = $html;
		return $results;
	}
